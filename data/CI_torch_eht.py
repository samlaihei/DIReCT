...
class Closure_Invariants():
    ...
    def FTCI(self, imgs, add_th_noise=False, th_noise_factor=1,
        ...
        for ind, (temp_vis, uv, pairs, time) in enumerate(zip(vis_by_time, uv_by_time, self.site_pairs, time_by_time)):
            time = time[:,0]
            if return_uv:
                temp_ci, temp_uv = self.ClosureInvariants(torch.tensor(temp_vis), uv=uv, pairs=pairs[0])
                out_uv.append(temp_uv)
            else:
                temp_ci, _ = self.ClosureInvariants(torch.tensor(temp_vis), uv=None, pairs=pairs[0])
    ...
    def ClosureInvariants(self, vis, uv=None, pairs=None):
        """
